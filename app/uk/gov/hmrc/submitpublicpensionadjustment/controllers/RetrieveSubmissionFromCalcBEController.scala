...
@Singleton
class RetrieveSubmissionFromCalcBEController @Inject() (
  ...
  def retrieveSubmissionStatus(submissionUniqueId: String): Action[AnyContent] = identify.async { implicit request =>
    val status = calculationDataService.retrieveSubmissionFromCalcBE(request.userId, submissionUniqueId)(ec, hc)
